...
const UserCards = () => {
    ...
    useEffect(async () => { await axios.post('https://mern-app-r.herokuapp.com/findAPI', {email : localStorage.getItem('email')}).then(async (e) => {
    ...
    const dele = async (e) => {
        ...
        if(chech==true)
        {
            console.log(name)
            console.log('hi')
        await axios.post('https://mern-app-r.herokuapp.com/delAPI', {name : name}).then((e) => {console.log(e)
